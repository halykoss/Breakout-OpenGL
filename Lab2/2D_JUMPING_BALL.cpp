...
mat4 Model; //Matrice per il cambiamento di sistema di riferimento: 
			// da Sistema diriferimento dell'oggetto a sistema di riferimento nel Mondo
...
Point* Punti = new Point[nVertices_montagna];
int vertices_Prato = 6;
Point* Prato = new Point[vertices_Prato];
...
//int nTriangles_PalaEolica = 8;
//int vertices_PalaEolica = 3 * nTriangles_PalaEolica + nTriangles_PalaEolica;
int nTriangles_PalaEolica = 4;
int vertices_PalaEolica = 3 * (nTriangles_PalaEolica + 2);
Point* PalaEolica = new Point[vertices_PalaEolica];
...
void disegna_PalaEolica(int nTriangles, Point* PalaEolica) {

	int i, cont;
	Point* Sostegno;
	int vertici = 6;
	Sostegno = new Point[vertici];
	...
	disegna_piano(-0.1, -1.0, 1.0, 1.0, col_nero, col_nero, Sostegno);
	...
	for (i = 0; i < 6; i++)
	{
		...
		PalaEolica[cont + i].z = Sostegno[i].z;
...
void disegna_montagne(float x0, float y0, int altezza_montagna, int larghezza_montagne, int numero_di_montagne, vec4 color_top, vec4 color_bot, Point* v_montagna)
{
	int i = 0;
...
void init(void)
{
	...
	disegna_piano(0, 0, 1, 1, col_bottom, col_top, Prato);
	//Genero un VAO Prato
	...
	glBufferData(GL_ARRAY_BUFFER, vertices_Prato * sizeof(Point), &Prato[0], GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 7 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 7 * sizeof(float), (void*)(3 * sizeof(float)));
	glEnableVertexAttribArray(1);
	...
	glBufferData(GL_ARRAY_BUFFER, nVertices_montagna * sizeof(Point), &Punti[0], GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 7 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 7 * sizeof(float), (void*)(3 * sizeof(float)));
	glEnableVertexAttribArray(1);
	...
	glBufferData(GL_ARRAY_BUFFER, vertices_PalaEolica * sizeof(Point), &PalaEolica[0], GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 7 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 7 * sizeof(float), (void*)(3 * sizeof(float)));
	glEnableVertexAttribArray(1);
...
//	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	...
	Model = mat4(1.0);
	float shadow_scale = lerp(1, 0, (float)distacco_da_terra / 255);
	if (shadow_scale < 0)
		shadow_scale = 0;
